...
| Date | Work |
|---|---|
| 2026-08-19 | **Bot first-run and permanent menu.** /start was a paragraph and a button, leaving no sign the bot could do anything; it now names every command and arrives with a persistent reply keyboard, so the commands are on screen from the first second instead of behind a slash nobody types. Telegram's menu button is EITHER the command list or the Mini App button, never both -- Open keeps the menu button, the commands moved to the keyboard, and the app is on that keyboard too as a real web_app button, so neither is given up. A keyboard tap arrives as ordinary text (the label), resolved to its command before anything else reads it, or plain-text capture would turn every press of "Vazifalar" into a task called "Vazifalar". Uzbek became the default rather than the fallback: nearly everyone is in Uzbekistan, many run Telegram in Russian, and many phones send no language tag -- English served the smallest group. Keyboard payload validated against the live API (a markup error names the button; "chat not found" means it was accepted). 7 mutations, one MISSED and the test was at fault -- `toContain('/today')` also matches `/todayX` -- now anchored on a word boundary. Suite 497 -> 504 |
| 2026-08-19 | **The bot does things now.** It could only answer "here is a button to open the app", which is a link with extra steps. Added /today (day at a glance), /tasks and /habits with inline buttons that complete the item in place, /add plus plain-text capture (any message becomes a task -- requiring a command first is exactly when capture stops being effortless). Buttons edit the message they came from, so ticking four habits leaves one correct message rather than five of history; only unticked habits get a button. Every id arriving in a callback is looked up scoped by userId, since callback data is client-controlled and Telegram ids are not secret. **Two silent failures, both found by testing the real thing rather than reading it:** `callback_query` was missing from allowed_updates, so buttons would render, spin and do nothing with nothing wrong in the code; and the hourly cron for the daily message made Vercel reject the whole deployment (Hobby allows daily crons only) -- a green push, no build, bot still on old code, no error anywhere until I deployed by hand. Now 03:00 UTC = 08:00 in UTC+5, with the per-timezone check preserved behind ?window=1 for Pro or an external scheduler; left on for a daily run it would have posted to nobody outside one timezone. Daily message is opt-in and switches itself off for anyone who blocks the bot. 17 mutations, 17 caught. Suite 465 -> 497 |
| 2026-08-19 | **Telegram Mini App + bot (@Daily_priority_bot).** The whole app now opens inside Telegram from the Open menu button -- same pages, same bottom nav, no separate build. Auth is the crux: Telegram hands the page a blob signed with the bot token, and that signature IS the authentication, so `verifyInitData` gets 19 tests with real signatures (fails closed with no token, rejects another bot's signature, a tampered user id, an injected field, and expires at 5 min). Its own NextAuth provider rather than a branch in the password one. Telegram ids kept as strings -- already past 32 bits, only promised to fit in 52, and it is a DB key. `env(safe-area-inset-*)` reports ZERO inside a Mini App (the webview sees no notch, and Telegram's header overlaps the top), so the real insets come over the JS bridge as CSS vars. Bot: /start /app /prayers /quran /streak /reminders /help, Uzbek for uz+ru clients, reusing the app's own streak definition and surah names so it cannot disagree with the screen. **The bot was dead on arrival and the code looked right**: middleware protects every /api/* with a session check and Telegram has no cookie, so every update 401'd before the handler -- found by curling the deployed URL, not by reading it. Token is in .env and Vercel only, with four guards scanning what git actually tracks; the scan itself was checked by planting a fake token, which walked past the first pattern because it pinned the secret to exactly 35 chars. Suite 436 -> 465 |
| 2026-08-19 | **Quran page deep-dive.** Reported as: the bookmark icon on each verse does nothing. The writes were landing the whole time -- a live row held `lastAyah: 40` from exactly those taps -- but they went to the reading POSITION, one moving pointer, so each tap replaced the last, the icon looked identical before and after, and there was nowhere to see what had been saved. Saved verses are a set now (`QuranBookmark`, unique per verse), the server owns the toggle so a stale client list cannot double-save, and they surface as jump-back chips on the list and an ayah strip in the reader. Reading the same live row exposed a second defect: `pagesRead: 604`, the entire Quran, because it was `max(page)` and An-Nas is page 604 -- opening the last surah once reported 100%. Pages are counted from rows now (`QuranPageRead`), with the client reporting which pages a sitting covered since only it has the ayah/page mapping; the old column stays (nothing dropped from a live DB) and is no longer read. That would have hidden the continue card for a truthful count of zero, so `hasPosition` became its own flag. Also: sajda verses marked (the flag shipped in the data on day one and was rendered nowhere), five-step Arabic text size remembered (it was hardcoded at 26px), copy-with-reference, the stopped-here marker, and a reader header that leads with the surah name. Verified in jsdom against the real component because the failure was that state never reached the screen; 15 mutations, 15 caught. Suite 411 -> 436 |
| 2026-08-17 | **Sidebar identity, and the Islamic content in Uzbek.** Quran and Ramadan read as the same tile because both gradients ended in `amber-600` and Quran's emerald start matched Prayers -- three of twelve tiles competing for one identity. Quran is now a mushaf binding (deep green into gold), Ramadan is night into lantern light ending cool-pink, and the other accented pages preview their own already-contrast-checked page accent, so a tile matches the screen it opens. **Quran names:** the Uzbek reader saw "Al-Baqara - The Cow" -- wrong language twice, on the one page that exists to be read in your own language. All 114 surahs got an Uzbek name and meaning following Tafsiri Hilol (the same translation the app already serves), the English article dropped the way an Uzbek mushaf writes it, kept out of the generated `surahs.ts`; four surfaces that each reached into `surah.en` now share one locale-aware helper, and search spans both languages because someone reading in Uzbek may still type "Baqarah". **The wider bug:** every Hijri string came from the API's `month.en` or a hardcoded English sentence -- "5 Rabi' al-Awwal 1448", "Eid al-Fitr" in the calendar grid, an untranslatable English special-day card, `AH` and `Loading...` hardcoded. All of it now passes the month NUMBER and a message KEY. **The keys already existed**: an earlier sweep had translated all twelve months and every special day into both dictionaries but left the code reading its own English table, so none was used -- my first pass added 34 duplicate keys before I found them. sessionStorage bumped to v4 so a payload without the new fields cannot make the holiday badge vanish. `isRamadan()` detected Ramadan by searching for "ramadan" in the formatted date, which in Uzbek ("Ramazon") would have answered false; it compares the month number now. 34 mutations, three missed and all three real gaps (a `` a heredoc had turned into a literal backspace; an assertion satisfied by a `title` while the visible badge rendered the raw key; a row asserting the helper somewhere rather than at the render site) -- tightened and re-run. **Reader scroll:** opening a surah after finishing one landed part-way down its text. The scroll ran inside the click handler one line after `setAyahs(...)`, so React had not committed -- and because the surah is cached for a year, on a repeat open `await fetch` resolved before the reader had rendered at all, leaving `readerRef.current` null and the `?.` swallowing it, so nothing scrolled and the window stayed where the LIST had been. Both pager buttons had the same defect. Now one effect keyed on surah+chunk that runs post-commit, plus `scroll-mt-24` for the `sticky top-0` header. Verified in jsdom by running the real component and recording what was on the page at each scroll -- a source check cannot tell a scroll that lands from one that no-ops on a null ref; four of five new tests fail against the old code. Suite 376 -> 411 |
