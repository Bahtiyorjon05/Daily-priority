...
export default function AccessibleDashboard({ session }: { session: any }) {
  ...
  const handleNewTaskShortcut = () => setShowNewTask(true)
  const handleToggleViewShortcut = () => setViewMode(viewMode === 'grid' ? 'list' : 'grid')
  const handleSearchFocusShortcut = () => searchInputRef.current?.focus()
  const handleAiSuggestShortcut = () => setAiLoading(true)
